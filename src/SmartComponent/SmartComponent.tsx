...
export function SmartComponent() {
    ...
    const headerSizePrompt = "Введите размер заголовка: 1 - большой, 2 - средний, 3 - маленький";
    ...
    const imageFloatPrompt = "Выберете выравнивание картинки: '0' - по левому краю; '1' - по правому краю; '2' - нет обтекания";
    ...
    const imageHandler = () => {
        ...
        float === "0" ? setImageFloat("left") : float === "1" ? setImageFloat("right") : setImageFloat("none");
    };

    return (
        <div className="container">
            <div className="button-container">
                <button className="button"onClick={headerHandler}>{headerText ? "Изменить заголовок": "Установить заголовок"}</button>
                <button className="button"onClick={collapsingBlockHandler}>
                    {collapsingBlockText ? "Изменить подзаголовок": "Установить подзаголовок"}
                </button>
                <button className="button"onClick={paragraphHandler}>{paragraphText ? "Изменить параграф" : "Добавить параграф"}</button>
                <button className="button"onClick={columnHandler}>{columnCount ? "Изменить колонки" : "Добавить колонки"}</button>
